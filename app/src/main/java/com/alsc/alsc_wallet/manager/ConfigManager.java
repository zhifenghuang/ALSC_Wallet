package com.alsc.alsc_wallet.manager;
...
import com.alsc.alsc_wallet.bean.chat.ChatSettingBean;
import com.alsc.alsc_wallet.bean.chat.ChatSubBean;
import com.alsc.alsc_wallet.bean.chat.FileBean;
...
import com.alsc.alsc_wallet.bean.chat.MessageResponse;
...
public class ConfigManager {
    ...
    private MyDialogFragment mLoginoutDialog, mGameInviteDialog;
    ...
    public void setContext(Context context) {
        mContext = context;
        Preferences.getInstacne().setContext(mContext);
        DatabaseOperate.setContext(mContext);
    ...
    class NetworkChangeReciver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (networkInfo != null && networkInfo.isAvailable()) {
                String token = DataManager.getInstance().getToken();
                if (!ConfigManager.getInstance().isConnected()) {
                    if (!TextUtils.isEmpty(token)) {
    ...
    public void initWebSocket(String token) {
        ...
        String url = String.format(Constants.CHAT_SOCKET_URL, token);
        //       Log.e(TAG, "url: " + url);
    ...
    public void showNotification(BasicMessage msg) {
        ...
        if (subBean.getNotInterupt() == 1) {  //消息免打扰
            return;
        }
    ...
    private void showNotification(Bitmap bmp, UserBean userBean, GroupBean groupBean, BasicMessage msg) {
        ...
        notification.icon = mContext.getResources().getIdentifier("alcs_logo", "mipmap", mContext.getPackageName());
        final RemoteViews remoteViews = new RemoteViews(mContext.getPackageName(), R.layout.layout_notification);
   //     remoteViews.setImageViewResource(R.id.ivAvatar, 0);
  ...
  //          remoteViews.setImageViewResource(R.id.ivAvatar, 0);
        ...
        intent.setComponent(new ComponentName(mContext.getPackageName(), "com.jnsh.alsc.SplashActivity"));
    ...
    public synchronized void showLoginOutDialog() {
        if (mLoginoutDialog != null || mCurrentActivity == null || mCurrentActivity.getLocalClassName().equals("WebviewActivity")) {
            return;
        }
        try {
...
//                        com.cao.commons.util.Utils.loginOutAndGoToType(mCurrentActivity);
//                    }
