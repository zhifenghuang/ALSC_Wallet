...
public class ChatUserAdapter extends BaseQuickAdapter<ChatBean, BaseViewHolder> {
    ...
    protected void convert(BaseViewHolder helper, ChatBean chatBean) {
        helper.getView(R.id.llParent).setBackgroundColor(ContextCompat.getColor(mContext, chatBean.isTopChat() ? R.color.color_f3_f7_fa : R.color.color_f3_f7_f9));
        if (chatBean.chatUser != null) {
            ...
            Utils.displayAvatar(mContext, R.drawable.chat_default_group_avatar, chatBean.chatUser.getAvatarUrl(), helper.getView(R.id.ivAvatar));
        } else {
