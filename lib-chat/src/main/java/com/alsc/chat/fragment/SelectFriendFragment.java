...
public class SelectFriendFragment extends ChatBaseFragment {
    ...
    protected void onViewCreated(View view) {
        ...
        if (mFromType == FROM_GROUP_DETAIL || mFromType == FROM_TRANSFER_GROUP ||
                mFromType == FROM_GROUP_ADD_BLOCK || mFromType == FROM_GROUP_REMOVE_BLOCK) {
            mGroup = (GroupBean) getArguments().getSerializable(Constants.BUNDLE_EXTRA_3);
        }
        ArrayList<UserBean> list = (ArrayList<UserBean>) getArguments().getSerializable(Constants.BUNDLE_EXTRA_2);
        ...
        if (mFromType == FROM_TRANSFER_GROUP || mFromType == FROM_GROUP_CHAT_AT) {
            setText(R.id.tvTitle, mFromType == FROM_TRANSFER_GROUP ? R.string.chat_select_transfer_group_user : R.string.chat_select_chat_at);
            ...
            if (mFromType == FROM_GROUP_ADD_BLOCK) {
                ...
                ArrayList<UserBean> groupUsers = (ArrayList<UserBean>) getArguments().getSerializable(Constants.BUNDLE_EXTRA_4);
                if (groupUsers != null && !groupUsers.isEmpty()) {
                    UserBean myInfo = DataManager.getInstance().getUser();
                    for (UserBean bean : groupUsers) {
                        if (bean.getUserId() == myInfo.getUserId()) {
                            groupUsers.remove(bean);
                            break;
                        }
                    }
                }
                getAdapter().setNewData(groupUsers);
        ...
        ChatHttpMethods.getInstance().inviteToGroup(mGroup.getGroupId(), list, new HttpObserver(new SubscriberOnNextListener() {
            ...
            public void onNext(Object o, String msg) {
                ...
                if (mGroup.getPayinState() == 1) {
                    ...
                    DataManager.getInstance().setObject(users);
                }
                goBack();
            }
        }, getActivity(), (BaseActivity) getActivity()));
