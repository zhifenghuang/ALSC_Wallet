...
public class GroupMessageAdapter extends MessageAdapter {
    ...
    protected void showUserInfo(final BaseViewHolder helper, BasicMessage item) {
        if (item.isMySendMsg(mMyInfo.getUserId())) {
            ...
            ivLeft.setOnLongClickListener(new View.OnLongClickListener() {
                ...
                public boolean onLongClick(View v) {
                    setOnItemChildLongClick(v, helper.getAdapterPosition());
                    return true;
                }
            });
            Utils.displayAvatar(mContext, R.drawable.chat_default_group_avatar, userBean == null ? "" : userBean.getAvatarUrl(), ivLeft);
